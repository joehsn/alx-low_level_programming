...
int _atoi(char *s)
{
	...
	/* Every piece of text ends with '\0' character, which indicates */
	...
	while (*s != '\0')
	{
		...
		if (*s > 47 && *s < 58)
		{
			...
			/* (num * 10) which results in adding 0 to the right of the number */
			/* then we add (*s - '0') the difference of `s` code, after */
			/* de-referencing it and the code of '0'. */
			num = (num * 10) + (*s - '0');
		}
